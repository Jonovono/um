...
exports.moveToSourcedDirectory = moveToSourcedDirectory;
...
function moveToSourcedDirectory(p) {
  var exists = fileExistsOnComputer(p);
  if (!exists) {
    log.error('A file with that name already exists in the sourced directory');
    return;
  }
  var basename = path.basename(p);
  var mvto = config.sources_path + '/' + basename;
