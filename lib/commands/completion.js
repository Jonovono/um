...
// This file is cool! Inspired by NPM.
// Basically you can now run umm completion and it will generate a file
// that will be sourced so you can add this to you .profile or whatever like:
// . <(umm completion). And then boom! You have autocomplete and can CD with it.
  ...
  if(args[0] === 'completion') return tabtab.complete('um', function(err, data) {
    ...
    UM.files.all(function(fs) {
      ...
      UM.tags.all(function(ts) {
        ...
        if (first === 'open') return tabtab.log(files, data);
        ...
        if (first === 'files') return tabtab.log(files, data);
