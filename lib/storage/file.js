...
// Takes an array of files and tags (and a comment, url, author)
// And adds all the tags to each files (and gives each file the comment etc tag)
File.prototype.updateFiles = function(files, tags, comment, url, author) {
  var that = this;
  _.each(files, function(file) {
    that.fileExistsInDatabase(file, function(exists) {
      if (!exists) {
        log.warn('File %s not tagged with tag(s) %s because the file has not been added to the database.',
                  colors.blue(file), colors.blue(tags));
        return;
      }
...
File.prototype.updateFile = function(file, tags, comment, url, author) {
  ...
  this.fileExistsInDatabase(basename, function(exists) {
    ...
    if (!exists) {
      log.warn('File %s does not exist in database. Cannot be updated', colors.blue(basename));
      return;
    }
...
File.prototype.tagFiles = function(files, tag) {
  var that = this;
  if (!helpers.hasAny(files)) return;
  _.each(files, function(file) {
    that.addTag(file, tag);
  });
};
...
File.prototype.addTags = function(file, tags) {
  ...
  this.storage.update({file: basename}, { $addToSet: {tags: { $each: tags}}}, {}, function() {
    log.info('%s Tag(s) %s has been added to file %s.', 
...
File.prototype.existingFilesFromFileList = function(files, cb) {
    var fileAndQuery = helpers.formatFileQuery(files);

  this.storage.find({ $or: fileAndQuery}, function(err, docs) {
    // console.log(docs);
    cb(docs);
  });
};
